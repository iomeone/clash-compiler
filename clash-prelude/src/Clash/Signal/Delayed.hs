...
import           Clash.Signal
  (HiddenClockReset, hideClockReset, Signal, delay, KnownDomain)
...
>>> let delay2 = delayedI :: HiddenClockReset tag enabled polarity dom => Int -> DSignal tag n Int -> DSignal tag (n + 2) Int
>>> let delayN2 = delayN d2
>>> let delayI2 = delayI :: HiddenClockReset tag enabled polarity dom => Int -> DSignal tag n Int -> DSignal tag (n + 2) Int
...
-- [-1,-1,1,2,3,4]
delayN
  :: forall tag dom enabled polarity a d n
   . ( HiddenClockReset tag enabled polarity dom
     , Undefined a )
  => SNat d
  -> a
...
-- [-1,-1,1,2,3,4]
delayI
  :: forall d n a tag enabled polarity dom
   . ( HiddenClockReset tag enabled polarity dom
     , Undefined a
     , KnownNat d )
  => a
...
-- [-1,-1,1,1,0,1,16,81]
delayedFold
  :: forall tag dom enabled polarity n delay k a
   . ( HiddenClockReset tag enabled polarity dom
     , Undefined a
     , KnownNat delay
     , KnownNat k )
