...
Clash has polarity 'Signal's in the form of:
...
and /tag/ is the /clock-/ (and /reset-/) tag to which the memory elements
manipulating these 'Signal's belong.

The type-parameter, /tag/, is of the kind 'Domain' which has types of the
following shape:

@
data tag = Dom { domainName :: 'GHC.TypeLits.Symbol', clkPeriod :: 'GHC.TypeLits.Nat' }
@

Where /domainName/ is a type-level string ('GHC.TypeLits.Symbol') representing
the name of the /clock-/ (and /reset-/) tag, and /clkPeriod/ is a type-level
natural number ('GHC.TypeLits.Nat') representing the clock period (in __ps__)
of the clock lines in the /clock-tag/.
...
offered by Clash's /tag/ annotation on 'Signal's is based on the uniqueness
...
meta-stability:
    ...
    *   /Reset situation 1/:
        ...
        @
        f :: Reset tag polarity -> Reset tag polarity -> ..
        f x y = ..
        @
    ...
    *   /Reset situation 2/:
        ...
        @
        g :: Reset tag Asynchronous -> Reset tag Asynchronous -> ..
        g x y = ..
        @

        This situation can be prone to metastability, because although /x/ and
        /y/ belong to the same /tag/ according to their type, there is no
    ...
    *   /Reset situation 3/:
        ...
        @
        h :: Reset tag Asynchronous -> Reset tag polarity -> ..
        h x y = ..
        @
        ...
        inducing metastability in the other component.
          ...
          * The Clash compiler will give a warning whenever a function has a
          type-signature similar to the one above.
          * Although in a standalone context, converting between @'Reset' tag
          'polarity'@ and @'Signal' tag 'Bool'@ would be safe from a
    ...
    *   /Clock situations 1, 2, and 3/:
        ...
        even though /x/ and /y/ belong to the same /tag/ according to their
...
resetSynchronizer
  ...
  -> Reset tag polarity
  -> Reset tag polarity
resetSynchronizer clk@(Clock tag _) rst@(toHighPolarity -> hRst) =
  case knownDomain tag of
    ...
    _ ->
      -- Reset is already polarity, nothing to do!
      rst
