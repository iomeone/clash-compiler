...
Clash has 'Signal's in the form of:
...
sample
  :: forall tag enabled dom a
   . ( KnownDomain tag dom
     , Undefined a )
  => (HiddenClockReset tag enabled 'ActiveHigh dom => Signal tag a)
  -- ^ 'Signal' we want to sample, whose source potentially has a hidden clock
  -- (and reset)
  -> [a]
sample s =
  let clk = unsafeCoerce @(Clock System 'Enabled)
                         @(Clock tag enabled)
                         (Clock @System SSymbol (Just (pure True)))
      rst = unsafeCoerce @(Reset System 'ActiveHigh)
                         @(Reset tag 'ActiveHigh)
                         (ActiveHighReset (True :- pure False))
...
-- __NB__: This function is not synthesizable
simulate
  :: forall tag enabled dom a b
   . ( KnownDomain tag dom
     , Undefined a
     , Undefined b )
  => (HiddenClockReset tag enabled 'ActiveHigh dom =>
...
testFor
  ...
  -> (HiddenClockReset tag enabled 'ActiveHigh dom => Signal tag Bool)
